...
export const Wallet = () => {
    const { setIsConnectModal , setWallet, setAddress } = GlobalContext()
    const wallets = [
        {
            name: 'Unisat Wallet',
    ...
    const UnisatConnect = async () => {
        ...
        try {
            ...
            setWallet('unisat')
            console.log('connect success', accounts,balanceRes);
    ...
    const OKXConnect = async () => {
        ...
        try {
            ...
            const res = await window.okxwallet.bitcoin.getAccounts();
            const balanceRes = await window.okxwallet.bitcoin.getBalance();
            await setAddress(res);
    ...
    return (
    <div className="inset-0 backdrop-blur-lg bg-clip-padding bg-opacity-60 fixed bg-black/80  w-[100%] z-[99999999] min-h-screen flex justify-center items-center">
        <div className="w-[94%] lg:w-[50%] h-auto py-3 px-3 drop-shadow-glow ml-auto mr-auto text-black  mt-[80px] bg-blue-600/80 rounded-3xl flex flex-col  pt-5 mb-20 ">
            <div className=" w-[95%] text-white ml-auto mr-auto flex h-12 mb-4 py-4 px-4">
                <p className="text-xl ml-0 mr-auto">Connect Your Wallet </p>
                <div className="text-xl mr-0 ml-auto">
                <div onClick={() => setIsConnectModal(false)} className='w-8 h-8 py-1.5 px-1 hover:bg-black/60 cursor-pointer rounded-lg bg-black/30'>
                  <TiCancelOutline className="ml-auto mr-auto"/> 
                </div>
                </div>
            </div>
            <div className="w-[95%] ml-auto mr-auto h-auto text-white  mb-4 py-4 px-4">
